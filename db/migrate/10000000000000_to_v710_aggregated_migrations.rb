...
class ToV710AggregatedMigrations < ActiveRecord::Migration[5.1]
  ...
  def raise_on_incomplete_3_0_migrations
    intersection = aggregated_versions_3_0 & all_versions

    if !intersection.empty? && intersection != aggregated_versions_3_0

      missing = aggregated_versions_3_0 - intersection
  ...
  def raise_on_incomplete_7_1_migrations
    ...
    if !intersection.empty? && intersection != aggregated_versions_7_1

      missing = aggregated_versions_7_1 - intersection

      # Only a part of the migrations that this migration aggregates
      # have already been applied. In this case, fail miserably.
      raise IncompleteMigrationsError, <<-MESSAGE.split("\n").map(&:strip!).join(' ') + "\n"
        It appears you are migrating from an incompatible version of
        OpenProject. Your database has only some migrations from OpenProject <
        v7.4.0. Please update your database to the schema of OpenProject
        v7.4.0 and run the OpenProject migrations again. The following
        migrations are missing: #{missing}
      MESSAGE
    end
  end
  ...
  def create_work_packages_table
  ...
  def create_workflows_table
    ...
    add_index 'workflows', ['role_id', 'type_id', 'old_status_id'], name: 'wkfs_role_tracker_old_status'
  ...
  def aggregated_versions_3_0
    @aggregated_versions_3_0 ||= self.class.migrations_to_3_0.split.map { |m|
      m.gsub(/_.*\z/, '').to_i
    }
  end

  def aggregated_versions_7_1
    @aggregated_versions_7_1 ||= self.class.migrations_to_7_1.split.map { |m|
      m.gsub(/_.*\z/, '').to_i
    }
  end
